...
while(conti == "s"):
    ...
    if op==("A"):
        ...
        print(f"({round(timer/7)} semanas y {timer%7} dias).")
        if cal==("s"):
            print(Fore.GREEN+"")
    ...
    if op==("C"):
        ...
        if pas_fut==("p"):
            while num>HOY:
                print(Fore.YELLOW+f"La cantidad introducida es superior al numero de dias transcurridos, el máximo es de{HOY-1}dias.")
        ...
        if pas_fut==("f"):
            while num>fut_hoy:
                print(Fore.YELLOW+f"La cantidad introducida es superior al numero de dias restantes, el máximo es de{fut_hoy}dias")
                num=OKI(input("Prueba con otro número: "))
            dist=HOY+num
            dateo=date.fromordinal(dist)
            date_spl=str(dateo).split("-")
